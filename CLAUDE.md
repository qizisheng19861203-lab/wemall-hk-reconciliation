...
- 订单同步、结算创建时自动打上当前激活店铺的 ID
- 订单列表、结算列表、统计报表均按激活店铺过滤
- 旧数据（切换前）全部标记为 `wemall_store_id=1`（蔚蓝医药主店）

### 产品库规则（重要）
- **产品库与店铺无关**，是所有店铺共用的母体产品库
- 同步微盟产品时，**永远固定使用蔚蓝医药（id=1）的凭证**，不随激活店铺切换
- 原因：倍赛思甄选有很多自己上架的产品，我们不需要；所有产品来源只来自蔚蓝医药
- 实现：`_get_wemall_master_api(db)` 函数，在 `products.py` 中 hardcode 查 `id=1`
...
### 清空旧店铺数据（待操作，正式切换时执行）
```sql
-- 清空蔚蓝医药主店的历史订单和结算（切换到倍赛思甄选后执行）
DELETE FROM order_items WHERE order_id IN (SELECT id FROM orders WHERE wemall_store_id = 1);
DELETE FROM orders WHERE wemall_store_id = 1;
DELETE FROM settlements WHERE wemall_store_id = 1;
```
...
### 产品管理（Products.vue）
- **仅管理员（admin）可见**，路由和侧栏菜单均有 `adminOnly` 守卫
...
backend/
  app/
    models/       数据库模型（User/Product/Order/Settlement/ExchangeRate/WemallStoreConfig）
    routers/      API路由（auth/products/orders/settlements/reports/wemall_stores）
  ...
  init_db.py      初始化管理员账号
frontend/
  src/views/      页面（Dashboard/Orders/Products/Settlements/Reports/Users/WemallStores/...）
nginx/
  nginx.conf      反向代理配置（监听 127.0.0.1:18080）
scripts/
...
- `GET  /api/orders` 订单列表（按激活店铺过滤）
- `POST /api/orders/sync-wemall` 从微盟同步订单（激活店铺）
...
- `GET  /api/reports/dashboard` 总览数据（按激活店铺）
- `GET  /api/reports/monthly-daily` 本月每日供货额（Dashboard图表用）
- `GET  /api/wemall-stores` 微盟店铺列表
...
- `WEMALL_APP_KEY/SECRET/SHOP_ID` 微盟API凭证（默认店铺，已迁移到 wemall_store_configs 表）
...
- 每 10 分钟自动同步一次
- 同步时自动打上激活店铺的 `wemall_store_id`

### 产品去重策略
- 同步时优先按 **SKU（outerGoodsCode）** 匹配已有产品
- 找不到 SKU 再按 **wemall_product_id** 匹配
- 都匹配不上才新增
- 找到后同步更新 wemall_product_id（防止 ID 漂移）
...
- 更新供货价后自动回填该产品所有历史订单中 supply_price 为空的条目
...
## 待办事项
- [x] 微盟API凭证已配置（迁移到 wemall_store_configs 表）
- [x] 汇率改用免费数据源
...
- [x] 多店铺隔离（orders/settlements 按 wemall_store_id 过滤）
- [x] 产品库仅管理员可见
...
// beforeEach：导航前立即清理（防止切换瞬间残留遮罩）
router.beforeEach(() => { cleanupOverlays() })

// afterEach：Vue DOM 更新后再清理一次（双保险）
router.afterEach(() => { nextTick(cleanupOverlays) })
```
...
```
已修复的文件：Settlements.vue / Orders.vue / Products.vue / Users.vue / ExchangeRates.vue / NotificationContacts.vue / Layout.vue（密码弹窗）/ WemallStores.vue

**第三层：CSS 强制 pointer-events:none（`frontend/src/assets/main.css`）**

⚠️ **El Plus 2.14 的正确过渡类名是 `dialog-fade-leave-active`，不是 `el-overlay-dialog-leave-active`**（之前写错导致修复无效！）

```css
/* El Plus 2.14 dialog 过渡名是 dialog-fade，离场期间禁用 pointer-events */
.dialog-fade-leave-active,
.dialog-fade-leave-to,
.el-overlay-dialog-leave-active,      /* 旧版兜底 */
.el-overlay-message-box-leave-active, /* 旧版兜底 */
.el-message-box__fade-leave-active,
.el-message-box__fade-leave-to {
...
### 搜索框 Enter 键问题
`el-input` 的 `@keyup.enter` 在 El Plus 2.x 某些情况不可靠。
正确写法：**不要用 `<form>` 包裹**（会干扰 dialog 的 Teleport），改用：
```html
<el-input v-model="keyword" @keydown.enter.prevent="load" @clear="load" />
...
# MySQL 不支持 ADD COLUMN IF NOT EXISTS，要用 try/except：
try:
